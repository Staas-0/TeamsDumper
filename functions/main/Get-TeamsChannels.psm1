function Get-TeamsChannels {
    [CmdletBinding()]
    param (
        [string]$exportFolder,
        [string]$clientId,
        [string]$clientSecret,
        [string]$tenantId,
        [array]$selectedTeams
    )

    ####################################
    ##   HTML  ##
    ####################################

    $chatHTMLTemplate = Get-Content -Raw ./assets/chat.html
    $messageHTMLTemplate = Get-Content -Raw ./assets/message.html
    $replyHTMLTemplate = Get-Content -Raw ./assets/reply.html
    $reactionHTMLTemplate = Get-Content -Raw ./assets/reaction.html
    $stylesheetCSS = Get-Content -Raw ./assets/stylesheet.css


    # Script
    $start = Get-Date
    Write-Verbose "Script start time: $start"

    $teamsMainFolder = Join-Path -Path $exportFolder -ChildPath "teams"
    if (-not(Test-Path -Path $teamsMainFolder)) { 
        New-Item -ItemType Directory -Path $teamsMainFolder | Out-Null 
        Write-Verbose "Created main teams export folder: $teamsMainFolder"
    }

    # Define the shared assets folder
    $sharedAssetsFolder = Join-Path -Path $exportFolder -ChildPath "assets"
    if (-not(Test-Path -Path $sharedAssetsFolder)) { 
        New-Item -ItemType Directory -Path $sharedAssetsFolder | Out-Null 
        Write-Verbose "Created shared assets folder: $sharedAssetsFolder"
    }

    # Loop through each team ID in selectedTeams
    foreach ($teamId in $selectedTeams) {
        Write-Verbose "Processing team ID: $teamId"

        $teamName = Invoke-RestMethod -Method Get -Uri "https://graph.microsoft.com/v1.0/teams/$teamId" -Headers @{
            "Authorization" = "Bearer $(Get-GraphAccessToken -clientId $clientId -clientSecret $clientSecret -tenantId $tenantId)"
        }
        $teamName = $teamName.displayName
        Write-Host "Processing channels for team: $teamName"
        Write-Verbose "Retrieved team name: $teamName"

        # Create a subfolder for the team
        $teamFolderName = $teamName
        $teamFolder = Join-Path -Path $teamsMainFolder -ChildPath $teamFolderName
        if (-not(Test-Path -Path $teamFolder)) { 
            New-Item -ItemType Directory -Path $teamFolder | Out-Null 
            Write-Verbose "Created team folder: $teamFolder"
        }

        $channels = Get-Channels -clientId $clientId -tenantId $tenantId -teamId $teamId
        Write-Host ("" + $channels.count + " possible channels found for team $teamName.")
        Write-Verbose "Retrieved $($channels.count) channels for team $teamName"

        $channelIndex = 0

        foreach ($channel in $channels) {
            Write-Progress -Activity "Exporting channels" -Status "team: $teamName - channel $($channelIndex) of $($channels.count)" -PercentComplete $(($channelIndex / $channels.count) * 100)
            $channelIndex += 1

            $channelName = $channel.displayName
            Write-Verbose "Processing channel: $channelName"

            $channelMessages = Get-ChannelMessages -teamId $teamId -channel $channel -clientId $clientId -tenantId $tenantId
            Write-Verbose "Retrieved messages for channel: $channelName"

            $messagesHTML = $null

            if (($channelMessages.count -gt 0) -and (-not([string]::isNullOrEmpty($channelName)))) {
                Write-Host -ForegroundColor White ("`r`n$channelName :: $($channelMessages.count) messages.")
                Write-Verbose "$($channelMessages.count) messages found for channel $channelName"

                Write-Host "Processing messages..."

                foreach ($channelMessage in $channelMessages) {
                    $profilePicture = Get-ProfilePicture $channelMessage.from.user.id $sharedAssetsFolder $clientId $tenantId
                    $time = ((Get-Date $channelMessage.createdDateTime -Format "dd MMMM yyyy, hh:mm tt") +" UTC")
                    Write-Verbose "Processing message created at $time by user $($channelMessage.from.user.id)"

                    switch ($channelMessage.messageType) {
                        "message" {
                            Write-Verbose "Processing regular message."
                            $channelMessageBody = Convert-Emoji $channelMessage.body.content

                            # Process embedded images
                            $imageTagMatches = [Regex]::Matches($channelMessageBody, "<img.+?src=[\`"']https:\/\/graph.microsoft.com(.+?)[\`"'].*?>")
                            foreach ($imageTagMatch in $imageTagMatches) {
                                Write-Verbose "Downloading embedded image in message..."
                                $imagePath = Get-Image $imageTagMatch $sharedAssetsFolder $clientId $tenantId
                                $channelMessageBody = $channelMessageBody.Replace($imageTagMatch.Groups[0], "<img src=`"$imagePath`" style=`"width: 100%;`" >")
                            }

                            $messageHTML = $messageHTMLTemplate
                            $messageHTML = $messageHTML.Replace("###ATTACHMENTS###", (ConvertTo-HTMLAttachments $channelMessage.attachments))
                            $messageHTML = $messageHTML.Replace("###CONVERSATION###", $channelMessageBody)
                            $messageHTML = $messageHTML.Replace("###DATE###", $time)
                            $messageHTML = $messageHTML.Replace("###DELETED###", "$($null -ne $channelMessage.deletedDateTime)".ToLower())
                            $messageHTML = $messageHTML.Replace("###EDITED###", "$($null -ne $channelMessage.lastEditedDateTime)".ToLower())
                            $messageHTML = $messageHTML.Replace("###IMAGE###", $profilePicture)
                            $messageHTML = $messageHTML.Replace("###NAME###", (Get-Initiator $channelMessage.from clientId $tenantId))
                            $messageHTML = $messageHTML.Replace("###PRIORITY###", $channelMessage.importance)

                            Write-Verbose "Processed message HTML for user $($channelMessage.from.user.id)"

                            $reactions = $channelMessage.reactions
                            $reactionHTML = ""

                            foreach ($reaction in $reactions) {
                                Write-Verbose "Processing reaction by $($reaction.user)"

                                $singleReactionHTML = $reactionHTMLTemplate
                                $reactionUser = $reaction.user
                                $reactionType = $reaction.reactionType
                                $reactionTime = ConvertTo-CleanDateTime $reaction.createdDateTime

                                $singleReactionHTML = $singleReactionHTML.Replace("###REACTION_NAME###", ((Get-Initiator $reactionUser clientId $tenantId) + " reacted with " + "$reactionType"))
                                $singleReactionHTML = $singleReactionHTML.Replace("###REACTION_DATE###", $reactionTime)
                                $reactionHTML += $singleReactionHTML
                            }
                            $messageHTML = $messageHTML.Replace("###REACTION###", $reactionHTML)

                            $replies = Get-ChannelMessageReplies -clientId $clientId -tenantId $tenantId -teamId $teamId -channelId $channel.id -messageId $channelMessage.id
                            Write-Verbose "Retrieved $($replies.count) replies for message."

                            $replyHTML = ""

                            foreach ($reply in $replies) {
                                Write-Verbose "Processing reply by user $($reply.from.user.id)"

                                $singleReplyHTML = $replyHTMLTemplate
                                $replyProfilePicture = Get-ProfilePicture $reply.from.user.id $sharedAssetsFolder $clientId $tenantId
                                $replyTime = ConvertTo-CleanDateTime $reply.createdDateTime

                                $singleReplyHTML = $singleReplyHTML.Replace("###REPLY_IMAGE###", $replyProfilePicture)
                                $singleReplyHTML = $singleReplyHTML.Replace("###REPLY_NAME###", ((Get-Initiator $reply.from clientId $tenantId) + " replied"))
                                $singleReplyHTML = $singleReplyHTML.Replace("###REPLY_DATE###", $replyTime)
                                $singleReplyHTML = $singleReplyHTML.Replace("###REPLY_CONTENT###", $reply.body.content)
                                $replyHTML += $singleReplyHTML
                            }
                            $messageHTML = $messageHTML.Replace("###REPLY###", $replyHTML)
                            $messagesHTML += $messageHTML
                            Break
                        }
                        "systemEventMessage" {
                            $systemImage = "data:image/webp;base64,"
                            Write-Verbose "Processing system event message."
                            $messageHTML = $messageHTMLTemplate
                            $messageHTML = $messageHTML.Replace("###ATTACHMENTS###", $null)
                            $messageHTML = $messageHTML.Replace("###CONVERSATION###", (ConvertTo-SystemEventMessage $channelMessage.eventDetail $clientId $tenantId))
                            $messageHTML = $messageHTML.Replace("###DATE###", $time)
                            $messageHTML = $messageHTML.Replace("###DELETED###", $null)
                            $messageHTML = $messageHTML.Replace("###EDITED###", $null)
                            $messageHTML = $messageHTML.Replace("###IMAGE###", $systemImage)
                            $messageHTML = $messageHTML.Replace("###NAME###", "System Event")
                            $messageHTML = $messageHTML.Replace("###PRIORITY###", $channelMessage.importance)
                            $messageHTML = $messageHTML.Replace("###REACTION###", "")
                            $messageHTML = $messageHTML.Replace("###REPLY###", "")
                            $messagesHTML += $messageHTML
                            Break
                        }
                        Default {
                            Write-Warning "Unhandled message type: $($channelMessage.messageType)"
                        }
                    }
                }

                $chatHTML = $chatHTMLTemplate
                $chatHTML = $chatHTML.Replace("###MESSAGES###", $messagesHTML)
                $chatHTML = $chatHTML.Replace("###CHATNAME###", $channelName)
                $chatHTML = $chatHTML.Replace("###STYLE###", $stylesheetCSS)

                $channelName = $channelName.Split([IO.Path]::GetInvalidFileNameChars()) -join "_"
                if ($channelName.length -gt 64) { $channelName = $channelName.Substring(0, 64) }

                $file = Join-Path -Path $teamFolder -ChildPath "$channelName.html"
                Write-Host -ForegroundColor Green "Exporting $file..."
                Write-Verbose "Exporting HTML file: $file"
                $chatHTML | Out-File -LiteralPath $file
            }
            else {
                Write-Host ("`r`n$channelName :: No messages found.")
                Write-Host -ForegroundColor Yellow "Skipping..."
                Write-Verbose "No messages found for channel $channelName; skipping."
            }
        }
    }

    # Finish up
    Write-Host "Finished processing channels in $((Get-Date) - $start)."
    Write-Verbose "Total processing time: $((Get-Date) - $start)"
}

Export-ModuleMember -Function Get-TeamsChannels